...
def new_page_function():
    ...
    if user_input:
        with col1:
            cortex_analyst_for_3rd_page(user_input)  # Run Cortex Analyst function with user input

        with col2:
            gpt4_query_for_3rd_page(user_input)  # Run GPT-4 function with user input
...
def gpt4_query_for_3rd_page(user_input):
    ...
    if user_input:
        ...
        result = session.sql(f"SELECT CHATGPT_4('{user_input}', '{json_data_str}')").collect()
        response = result[0][0]
...
def main():
    ...
    page_selection = st.sidebar.radio("Go to", ["Cortex Analyst", "GPT-4 Query Interface", "Compare Models"])
...
def generate_metadata_string(session):
    ...
    for ddl in ddls:
        table_name = ddl.split('TABLE')[1].split('(')[0].strip()  # Extract table name from DDL statement
        metadata_lines.append(f"\nTable: {table_name}\n")
        metadata_lines.append(f"{ddl}\n")
        
        # Fetch column details for the current table
        column_details = fetch_column_details(database_name, schema_name, table_name, session)
        
        # Add column metadata
        for column in column_details:
            metadata_lines.append(f"Column: {column['column_name']}")
            metadata_lines.append(f"  - Unique Count: {column['unique_count']}")
            metadata_lines.append(f"  - Random Unique Values: {column['random_values']}\n")
